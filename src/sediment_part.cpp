...
sediment_part::sediment_part(lexer* p, ghostcell* pgc, turbulence *pturb) : PP(p->S20,p->S22,true), PP2(p->S20,p->S22,true), active_box(p), active_box_dummy(p), active_topo(p), irand(10000), drand(irand), s(p), pbedshear(p,pturb), prelax(p), pslope(p)
{
