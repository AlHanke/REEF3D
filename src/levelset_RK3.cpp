...
void levelset_RK3::start(fdm* a, lexer* p, convection* pconvec, solver*, ghostcell* pgc, ioflow* pflow, reini* preini, particle_corr* ppart, field &ls)
{
    ...
    double setValtime, pconvectime, calctime, phirelaxtime, gctime, solidforcetime, parttime, reinitime, picardtime, parttime2, update_time;
    double temptime;
...
// Step 3
    ...
    if(p->mpirank==0 && p->count%p->P12==0 && p->count>0)
    {
        ...
        <<"\treinitime:   "<<std::setprecision(precision)<<reinitime-parttime<<"\n"
        <<"\tupdate_time: "<<std::setprecision(precision)<<update_time-parttime2<<"\n"<<std::endl;
    }
}
