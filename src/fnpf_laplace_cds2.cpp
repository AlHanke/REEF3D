...
void fnpf_laplace_cds2::start(lexer* p, fdm_fnpf *c, ghostcell *pgc, solver *psolv, fnpf_fsf *pf, double *f, slice &Fifsf)
{
    ...
    LOOP
    {
        ...
        if(p->j_dir)
        {
            c->M.w[n] = -1.0/(p->DYP[JP]*p->DYN[JP]);
            c->M.e[n] = -1.0/(p->DYP[JM1]*p->DYN[JP]);
        }
        else
        {
            c->M.w[n] = 0.0;
    ...
    LOOP
    {
        if(p->wet[IJ]==1 && p->flag7[FIJK]>0)
        {
            ...
            else if(p->B98>2)
            {
                ...
                else if(p->flag7[FIm1JK]<0 && c->bc(i-1,j)==1 && p->A329>=2)
                {
            ...
            else if(p->B99>2)
            {
                ...
                else if(p->flag7[FIp1JK]<0 && c->bc(i+1,j)==2 && p->A329>=2)
                {
                    if(p->i_dir)
                    {
                        c->rhsvec.V[n] -= 2.0*p->sigx[FIJK]*(f[FIp1JKp1] - f[FIm1JKp1] - f[FIp1JKm1] + f[FIm1JKm1])
