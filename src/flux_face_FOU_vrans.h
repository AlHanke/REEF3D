...
private:
    ...
    void u_flux_impl(fdm *a, int ipol, const GenericField &uvel, double &uflux1, double &uflux2) const
    {
        if(ipol==1)
        {
            if(p->flag1[Im1JK]>0)
    ...
    template<typename GenericField>
    void v_flux_impl(fdm *a, int ipol, const GenericField &vvel, double &vflux1, double &vflux2) const
    {
        if(ipol==1)
        {
    ...
    template<typename GenericField>
    void w_flux_impl(fdm *a, int ipol, const GenericField &wvel, double &wflux1, double &wflux2) const
    {
        if(ipol==1)
        {
