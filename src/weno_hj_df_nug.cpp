...
void weno_hj_df_nug::start(lexer *p, fdm *a, field &b, int ipol, field &uvel, field &vvel, field &wvel)
{
    ...
    if(ipol==1)
    {
        ...
        FIELDLOOP_INC_MEMBER(a,F,
            ...
            F(i,j,k)+=aij(flux,p,a,b,1,uvel,vvel,wvel,p->DXN.data(),p->DYP.data(),p->DZP.data());
        )
    }
    else if(ipol==2 && p->j_dir==1)
    {
        ...
        FIELDLOOP_INC_MEMBER(a,G,
            FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
            G(i,j,k)+=aij(flux,p,a,b,2,uvel,vvel,wvel,p->DXP.data(),p->DYN.data(),p->DZP.data());
        )
    }
    else if(ipol==3)
    {
        ...
        FIELDLOOP_INC_MEMBER(a,H,
            FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
            H(i,j,k)+=aij(flux,p,a,b,3,uvel,vvel,wvel,p->DXP.data(),p->DYP.data(),p->DZN.data());
        )
    }
    else if(ipol==4)
    {
        FIELDLOOP_INC_MEMBER(a,L,
            FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
            L(i,j,k)+=aij(flux,p,a,b,4,uvel,vvel,wvel,p->DXP.data(),p->DYP.data(),p->DZP.data());
        )
    }
    }, pflux);
}
...
double weno_hj_df_nug::aij(FluxT &pflux, lexer *p, fdm *a, const GenericField &b, int ipol, const GenericField &uvel, const GenericField &vvel, const GenericField &wvel, double *DX, double *DY, double *DZ)
{
...
double weno_hj_df_nug::fx(lexer* p, fdm* a, const GenericField& b, const double* DX, double advec)
{
    ...
    if(advec>0.0)
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i-2,j,k)+a->fbh5(i-3,j,k))*(b(i-2,j,k)-b(i-3,j,k))/DX[IM3];
            q2 = 0.5*(a->fbh5(i-1,j,k)+a->fbh5(i-2,j,k))*(b(i-1,j,k)-b(i-2,j,k))/DX[IM2];
            q3 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i-1,j,k))*(b(i,j,k)  -b(i-1,j,k))/DX[IM1];
            q4 = 0.5*(a->fbh5(i+1,j,k)+a->fbh5(i,j,k)  )*(b(i+1,j,k)-b(i,j,k)  )/DX[IP];
            q5 = 0.5*(a->fbh5(i+2,j,k)+a->fbh5(i+1,j,k))*(b(i+2,j,k)-b(i+1,j,k))/DX[IP1];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i-3,j,k)*(b(i-2,j,k)-b(i-3,j,k))/DX[IM3];
            q2 = a->fbh5(i-2,j,k)*(b(i-1,j,k)-b(i-2,j,k))/DX[IM2];
            q3 = a->fbh5(i-1,j,k)*(b(i,j,k)  -b(i-1,j,k))/DX[IM1];
            q4 = a->fbh5(i,j,k)  *(b(i+1,j,k)-b(i,j,k)  )/DX[IP];
            q5 = a->fbh5(i+1,j,k)*(b(i+2,j,k)-b(i+1,j,k))/DX[IP1];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
    ...
    else
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i-1,j,k)+a->fbh5(i-2,j,k))*(b(i-1,j,k)-b(i-2,j,k))/DX[IM2];
            q2 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i-1,j,k))*(b(i,j,k)  -b(i-1,j,k))/DX[IM1];
            q3 = 0.5*(a->fbh5(i+1,j,k)+a->fbh5(i,j,k)  )*(b(i+1,j,k)-b(i,j,k)  )/DX[IP];
            q4 = 0.5*(a->fbh5(i+2,j,k)+a->fbh5(i+1,j,k))*(b(i+2,j,k)-b(i+1,j,k))/DX[IP1];
            q5 = 0.5*(a->fbh5(i+3,j,k)+a->fbh5(i+2,j,k))*(b(i+3,j,k)-b(i+2,j,k))/DX[IP2];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i-1,j,k)*(b(i-1,j,k)-b(i-2,j,k))/DX[IM2];
            q2 = a->fbh5(i,j,k)  *(b(i,j,k)  -b(i-1,j,k))/DX[IM1];
            q3 = a->fbh5(i+1,j,k)*(b(i+1,j,k)-b(i,j,k)  )/DX[IP];
            q4 = a->fbh5(i+2,j,k)*(b(i+2,j,k)-b(i+1,j,k))/DX[IP1];
            q5 = a->fbh5(i+3,j,k)*(b(i+3,j,k)-b(i+2,j,k))/DX[IP2];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
...
template<typename GenericField>
double weno_hj_df_nug::fy(lexer* p, fdm* a, const GenericField& b, const double* DY, double advec)
{
    if(advec==0.0) return 0.0;
    ...
    if(advec>0.0)
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i,j-2,k)+a->fbh5(i,j-3,k))*(b(i,j-2,k)-b(i,j-3,k))/DY[JM3];
            q2 = 0.5*(a->fbh5(i,j-1,k)+a->fbh5(i,j-2,k))*(b(i,j-1,k)-b(i,j-2,k))/DY[JM2];
            q3 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i,j-1,k))*(b(i,j,k)  -b(i,j-1,k))/DY[JM1];
            q4 = 0.5*(a->fbh5(i,j+1,k)+a->fbh5(i,j,k)  )*(b(i,j+1,k)-b(i,j,k)  )/DY[JP];
            q5 = 0.5*(a->fbh5(i,j+2,k)+a->fbh5(i,j+1,k))*(b(i,j+2,k)-b(i,j+1,k))/DY[JP1];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i,j-3,k)*(b(i,j-2,k)-b(i,j-3,k))/DY[JM3];
            q2 = a->fbh5(i,j-2,k)*(b(i,j-1,k)-b(i,j-2,k))/DY[JM2];
            q3 = a->fbh5(i,j-1,k)*(b(i,j,k)  -b(i,j-1,k))/DY[JM1];
            q4 = a->fbh5(i,j,k)  *(b(i,j+1,k)-b(i,j,k)  )/DY[JP];
            q5 = a->fbh5(i,j+1,k)*(b(i,j+2,k)-b(i,j+1,k))/DY[JP1];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
    ...
    else
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i,j-1,k)+a->fbh5(i,j-2,k))*(b(i,j-1,k)-b(i,j-2,k))/DY[JM2];
            q2 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i,j-1,k))*(b(i,j,k)  -b(i,j-1,k))/DY[JM1];
            q3 = 0.5*(a->fbh5(i,j+1,k)+a->fbh5(i,j,k)  )*(b(i,j+1,k)-b(i,j,k)  )/DY[JP];
            q4 = 0.5*(a->fbh5(i,j+2,k)+a->fbh5(i,j+1,k))*(b(i,j+2,k)-b(i,j+1,k))/DY[JP1];
            q5 = 0.5*(a->fbh5(i,j+3,k)+a->fbh5(i,j+2,k))*(b(i,j+3,k)-b(i,j+2,k))/DY[JP2];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i,j-1,k)*(b(i,j-1,k)-b(i,j-2,k))/DY[JM2];
            q2 = a->fbh5(i,j,k)  *(b(i,j,k)  -b(i,j-1,k))/DY[JM1];
            q3 = a->fbh5(i,j+1,k)*(b(i,j+1,k)-b(i,j,k)  )/DY[JP];
            q4 = a->fbh5(i,j+2,k)*(b(i,j+2,k)-b(i,j+1,k))/DY[JP1];
            q5 = a->fbh5(i,j+3,k)*(b(i,j+3,k)-b(i,j+2,k))/DY[JP2];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
...
template<typename GenericField>
double weno_hj_df_nug::fz(lexer* p, fdm* a, const GenericField& b, const double* DZ, double advec)
{
    if(advec==0.0) return 0.0;
    ...
    if(advec>0.0)
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i,j,k-2)+a->fbh5(i,j,k-3))*(b(i,j,k-2)-b(i,j,k-3))/DZ[KM3];
            q2 = 0.5*(a->fbh5(i,j,k-1)+a->fbh5(i,j,k-2))*(b(i,j,k-1)-b(i,j,k-2))/DZ[KM2];
            q3 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i,j,k-1))*(b(i,j,k)  -b(i,j,k-1))/DZ[KM1];
            q4 = 0.5*(a->fbh5(i,j,k+1)+a->fbh5(i,j,k)  )*(b(i,j,k+1)-b(i,j,k)  )/DZ[KP];
            q5 = 0.5*(a->fbh5(i,j,k+2)+a->fbh5(i,j,k+1))*(b(i,j,k+2)-b(i,j,k+1))/DZ[KP1];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i,j,k-3)*(b(i,j,k-2)-b(i,j,k-3))/DZ[KM3];
            q2 = a->fbh5(i,j,k-2)*(b(i,j,k-1)-b(i,j,k-2))/DZ[KM2];
            q3 = a->fbh5(i,j,k-1)*(b(i,j,k)  -b(i,j,k-1))/DZ[KM1];
            q4 = a->fbh5(i,j,k)  *(b(i,j,k+1)-b(i,j,k)  )/DZ[KP];
            q5 = a->fbh5(i,j,k+1)*(b(i,j,k+2)-b(i,j,k+1))/DZ[KP1];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
    ...
    else
    {
        ...
        {
            q1 = 0.5*(a->fbh5(i,j,k-1)+a->fbh5(i,j,k-2))*(b(i,j,k-1)-b(i,j,k-2))/DZ[KM2];
            q2 = 0.5*(a->fbh5(i,j,k)  +a->fbh5(i,j,k-1))*(b(i,j,k)  -b(i,j,k-1))/DZ[KM1];
            q3 = 0.5*(a->fbh5(i,j,k+1)+a->fbh5(i,j,k)  )*(b(i,j,k+1)-b(i,j,k)  )/DZ[KP];
            q4 = 0.5*(a->fbh5(i,j,k+2)+a->fbh5(i,j,k+1))*(b(i,j,k+2)-b(i,j,k+1))/DZ[KP1];
            q5 = 0.5*(a->fbh5(i,j,k+3)+a->fbh5(i,j,k+2))*(b(i,j,k+3)-b(i,j,k+2))/DZ[KP2];
        }
        else if(p->X45==3)
        {
            q1=q2=q3=q4=q5=0.0;
        ...
        {
            q1 = a->fbh5(i,j,k-1)*(b(i,j,k-1)-b(i,j,k-2))/DZ[KM2];
            q2 = a->fbh5(i,j,k)  *(b(i,j,k)  -b(i,j,k-1))/DZ[KM1];
            q3 = a->fbh5(i,j,k+1)*(b(i,j,k+1)-b(i,j,k)  )/DZ[KP];
            q4 = a->fbh5(i,j,k+2)*(b(i,j,k+2)-b(i,j,k+1))/DZ[KP1];
            q5 = a->fbh5(i,j,k+3)*(b(i,j,k+3)-b(i,j,k+2))/DZ[KP2];
        }
        else if(p->X45==5)
        {
            q1=q2=q3=q4=q5=0.0;
