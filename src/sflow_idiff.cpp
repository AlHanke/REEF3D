...
void sflow_idiff::diff_u(lexer* p, fdm2D *b, ghostcell *pgc, solver2D *psolv, slice &u, slice &v, double alpha)
{
    ...
    SLICELOOP1
    {
        ...
        b->M.p[n] =   4.0*visc/(p->DXM*p->DXM)
                    ...
                    + (p->j_dir ? 2.0*visc/(p->DXM*p->DXM) : 0.0)
        ...
        if(p->j_dir)
        {
            b->M.e[n] = -visc/(p->DXM*p->DXM);
            b->M.w[n] = -visc/(p->DXM*p->DXM);
        }
        else
        {
            b->M.e[n] = 0.0;
...
void sflow_idiff::diff_v(lexer* p, fdm2D *b, ghostcell *pgc, solver2D *psolv, slice &u, slice &v, double alpha)
{
    ...
    SLICELOOP2
    {
        ...
        b->M.p[n] =   2.0*visc/(p->DXM*p->DXM)
                    ...
                    + (p->j_dir ? 4.0*visc/(p->DXM*p->DXM) : 0.0)
        ...
        if(p->j_dir)
        {
            b->M.e[n] = -2.0*visc/(p->DXM*p->DXM);
            b->M.w[n] = -2.0*visc/(p->DXM*p->DXM);
        }
        else
        {
            b->M.e[n] = 0.0;
...
void sflow_idiff::diff_scalar(lexer* p, fdm2D *b, ghostcell *pgc, solver2D *psolv, slice &f, double sig, double alpha)
{
    ...
    SLICELOOP4
    {
        ...
        b->M.p[count]  += 0.5*sqd*(visc_ij+b->eddyv(i+1,j)/sig + visc_ij+ev_ij/sig)
                        ...
                        + (p->j_dir ? 0.5*sqd*(visc_ij+b->eddyv(i,j+1)/sig + visc_ij+ev_ij/sig)

                        + 0.5*sqd*(visc_ij+ev_ij/sig + visc_ij+b->eddyv(i,j-1)/sig) : 0.0);
        ...
        if(p->j_dir)
        {
            b->M.e[count] -= 0.5*sqd*(visc_ij+ev_ij/sig + visc_ij+b->eddyv(i,j-1)/sig);
            b->M.w[count] -= 0.5*sqd*(visc_ij+b->eddyv(i,j+1)/sig + visc_ij+ev_ij/sig);
        }
        else
        {
            b->M.e[count] = 0.0;
