                      ...
                      ioflow* pflow, vrans* pvrans, sixdof *p6dof, vector<net*>& pnet)
{
    ...
    pflow->rkinflow(p,a,pgc,urk1,vrk1,wrk1);
        flow->rkinflow(p,a,pgc,urk2,vrk2,wrk2);
...
//Step 2
    ...
    ULOOP
                2(i,j,k) = 0.75*a->u(i,j,k) + 0.25*urk1(i,j,k)
    ...
    VLOOP
                2(i,j,k) = 0.75*a->v(i,j,k) + 0.25*vrk1(i,j,k)
    ...
    WLOOP
                2(i,j,k) = 0.75*a->w(i,j,k) + 0.25*wrk1(i,j,k)
...
//--------------------------------------------------------
    ...
    ULOOP
                u(i,j,k) = (1.0/3.0)*a->u(i,j,k) + (2.0/3.0)*urk2(i,j,k)
    ...
    VLOOP
                v(i,j,k) = (1.0/3.0)*a->v(i,j,k) + (2.0/3.0)*vrk2(i,j,k)
    ...
    WLOOP
                w(i,j,k) = (1.0/3.0)*a->w(i,j,k) + (2.0/3.0)*wrk2(i,j,k)
