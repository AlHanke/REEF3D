...
public:
    ...
    inline int& operator()(int ii, int jj, int kk) final
    {
        refresh_cache_if_needed();
        return m_cached_arr4(ii + m_cached_ox, jj + m_cached_oy, kk + m_cached_oz, 0);
    }

    inline const int& operator()(int ii, int jj, int kk) const final
    {
        refresh_const_cache_if_needed();
        return m_cached_const_arr4(ii + m_cached_const_ox, jj + m_cached_const_oy, kk + m_cached_const_oz, 0);
    }

    inline int& operator()(const amrex::IntVect& iv, int comp = 0) final
    {
        refresh_cache_if_needed();
        return m_cached_arr4(iv, comp);
    }

    inline const int& operator()(const amrex::IntVect& iv, int comp = 0) const final
    {
        refresh_const_cache_if_needed();
...
private:
    ...
    AMREX_FORCE_INLINE void refresh_cache_if_needed()
    {
        ...
        {
            m_cached_arr4    = mf[cur_lev].array(*(p->amr_cell_mfi));
    ...
    AMREX_FORCE_INLINE void refresh_const_cache_if_needed() const
    {
        ...
        {
            m_cached_const_arr4    = mf[cur_lev].const_array(*(p->amr_cell_mfi));
