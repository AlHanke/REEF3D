...
surftens::surftens(lexer* p) : gradient(p), epsi(p->F45*p->DXM), tension(p->W5), tensionCheck(tension>1.0e-10), levelSetCheck(p->F30!=0 || p->F40!=0)
{
}
...
void surftens::surface_tension(lexer *p, fdm *a, field &surf, int gcval)
{
    if(tensionCheck)
    {
        ...
        if(gcval==10)
        {
            ULOOP
            {
                if(levelSetCheck && fabs(0.5*(a->phi(i,j,k)+a->phi(i+1,j,k)))<epsi)
                {
                    double dirac = (0.5/epsi)*(1.0 + cos((PI*0.5*(a->phi(i,j,k)+a->phi(i+1,j,k)))/epsi));

                    double curv = (pow(xdx(a,a->phi),2.0)*(xdyy(a,a->phi) + xdzz(a,a->phi))
        ...
        else if(gcval==11)
        {
            VLOOP
            {
                if(levelSetCheck && fabs(0.5*(a->phi(i,j,k)+a->phi(i,j+1,k)))<epsi)
                {
                    double dirac = (0.5/epsi)*(1.0 + cos((PI*0.5*(a->phi(i,j,k)+a->phi(i,j+1,k)))/epsi));

                    double curv = (pow(ydx(a,a->phi),2.0)*(ydyy(a,a->phi) + ydzz(a,a->phi))
        ...
        else if(gcval==12)
        {
            WLOOP
            {
                if(levelSetCheck && fabs(0.5*(a->phi(i,j,k)+a->phi(i,j,k+1)))<epsi)
                {
                    double dirac = (0.5/epsi)*(1.0 + cos((PI*0.5*(a->phi(i,j,k)+a->phi(i,j,k+1)))/epsi));

                    double curv = (pow(zdx(a,a->phi),2.0)*(zdyy(a,a->phi) + zdzz(a,a->phi))
