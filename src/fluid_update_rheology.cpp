...
void fluid_update_rheology::start(lexer *p, fdm* a, ghostcell* pgc)
{
    ...
    {  
        ...
        if(p->flagsf4[IJK]>0)
        {
        p->volume1 += p->DXN[IP]*p->DYN[JP]*p->DZN[KP]*(H_phi-(1.0-a->porosity(i,j,k)))*0.305/p->DYN[JP];
        p->volume2 += p->DXN[IP]*p->DYN[JP]*p->DZN[KP]*(1.0-H_phi-(1.0-a->porosity(i,j,k)))*0.305/p->DYN[JP];
        }
    }
