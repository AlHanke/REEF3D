...
void fluid_update_rheology::start(lexer *p, fdm* a, ghostcell* pgc)
{
	...
	// density, viscosity & multiphase volumes
	LOOP
	{  
		if(a->phi(i,j,k)>epsi)
		{
        ...
        p->volume1 += p->DXN[IP]*p->DYN[JP]*p->DZN[KP]*(H-(1.0-a->porosity(i,j,k)));
        p->volume2 += p->DXN[IP]*p->DYN[JP]*p->DZN[KP]*(1.0-H-(1.0-a->porosity(i,j,k)));
	}
