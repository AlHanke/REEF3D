...
void grid_helper::fill_dgc3(lexer* p)
{
    int q, count;

    QGC3LOOP
    {
        i=p->gcb3[p->level][q].i;
        j=p->gcb3[p->level][q].j;
        k=p->gcb3[p->level][q].k;

        if(p->gcb3[p->level][q].cs==X_NEG)
            for(n=0;n<p->margin;++n)
            hgc[(i-imin-n-1)*jmax*kmax + (j-jmin)*kmax + k-kmin]+=1;

        else if(p->gcb3[p->level][q].cs==X_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin+n+1)*jmax*kmax + (j-jmin)*kmax + k-kmin]+=1;

        else if(p->gcb3[p->level][q].cs==Y_NEG)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin-n-1)*kmax + k-kmin]+=1;

        else if(p->gcb3[p->level][q].cs==Y_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin+n+1)*kmax + k-kmin]+=1;

        else if(p->gcb3[p->level][q].cs==Z_NEG)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin)*kmax + k-kmin-n-1]+=1;

        else if(p->gcb3[p->level][q].cs==Z_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin)*kmax + k-kmin+n+1]+=1;
    }
    ...
    p->Iresize(p->dgc3,p->dgc3_count,count,6,6);
