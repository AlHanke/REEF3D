...
void ghostcell::gcb_distbed(lexer *p, fdm *a)
{
    double nx,ny,nz,norm;

    for(n=p->gcb_solid;n<p->gcb4_count;++n)
    {
        i=p->gcb4[n][0];
        j=p->gcb4[n][1];
        k=p->gcb4[n][2];
        ...
        if(p->gcb4[n][3]==1)
        p->gcd4[n]=fabs(a->topo(i,j,k))/(fabs(nx)>1.0e-15?fabs(nx):1.0e-15);

        if(p->gcb4[n][3]==4)
        p->gcd4[n]=fabs(a->topo(i,j,k))/(fabs(nx)>1.0e-15?fabs(nx):1.0e-15);

        if(p->gcb4[n][3]==3)
        p->gcd4[n]=fabs(a->topo(i,j,k))/(fabs(ny)>1.0e-15?fabs(ny):1.0e-15);
