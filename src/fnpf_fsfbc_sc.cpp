...
fnpf_fsfbc_sc::~fnpf_fsfbc_sc()
{
}

void fnpf_fsfbc_sc::fsfdisc(lexer *p, fdm_fnpf *c, ghostcell *pgc, slice &eta, slice &Fifsf)
{
    SLICELOOP4
...
void fnpf_fsfbc_sc::fsfdisc_ini(lexer *p, fdm_fnpf *c, ghostcell *pgc, slice &eta, slice &Fifsf)
{
    FFILOOP4
    {
    c->Bx(i,j) = pdx->sx(p,c->depth,1.0);
...
void fnpf_fsfbc_sc::fsfwvel(lexer *p, fdm_fnpf *c, ghostcell *pgc, slice &eta, slice &Fifsf)
{
    // fi
    FFILOOP4
    {
...
void fnpf_fsfbc_sc::kfsfbc(lexer *p, fdm_fnpf *c, ghostcell *pgc)
{
    SLICELOOP4
    {
...
void fnpf_fsfbc_sc::dfsfbc(lexer *p, fdm_fnpf *c, ghostcell *pgc, slice &eta)
{
    SLICELOOP4
    {
