...
ghostcell::bc_labels ghostcell::gcsleval1(int gcv, int bc, int cs)
{
    ...
    // Wall
    // Parallel
    else if((bc==5 || bc==7 || bc==21 || bc==22) && (cs==Y_POS || cs==Y_NEG || cs==Z_NEG || cs==Z_POS) && gcv==10)
        return gclabel_u;
    // Orthogonal
    else if((bc==3 || bc==5 || (bc==7 && !awa_label) || bc==21 || bc==22) && (cs==X_NEG || cs==X_POS) && gcv==10)
        return bc_labels::NOSLIP;
    // Outflow
    else if(bc==2 && (cs==X_NEG || cs==X_POS) && gcv==10)
        return bc_labels::OUTFLOW;
    // Symmetry
    else if(bc==3 && (cs==Y_POS || cs==Y_NEG || cs==Z_NEG || cs==Z_POS) && gcv==10)
        return bc_labels::NEUMANN;
    // Hx
