...
void printer_CFD::print3DMPI(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    ...
    if(p->P15==2)
        num = p->count;
    sprintf(name,"./REEF3D_CFD_VTRMPI/REEF3D-CFD-%08i.vtr",num);
...
void printer_CFD::writeHeader(lexer *p, fdm *a, ghostcell *pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed, MPI_File &file, int *piextent)
{
    ...
    if(MPIinitialPrint)
        MPI_Bcast(offsetMPI, p->mpi_size, MPI_OFFSET, 0, MPI_COMM_WORLD);
}
...
void printer_CFD::writeData(lexer *p, fdm *a, ghostcell *pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed, MPI_File &file)
{
    ...
    MPI_File_write_at_all(file, offsetMPI[p->mpirank], data.data(), data.size(), MPI_FLOAT, MPI_STATUS_IGNORE);
}
