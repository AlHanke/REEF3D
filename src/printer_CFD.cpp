...
void printer_CFD::prepMPIprint(lexer *p, fdm* a, ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    if(p->mpirank==0)
    mkdir("./REEF3D_CFD_VTRMPI",0777);
    offsetsComp(p,a,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed);
    int *recvcounts=nullptr;
    if(p->mpirank==0)
    recvcounts = (int *)malloc(p->mpi_size*sizeof(int));
    pgc->gather_int(&n,1,recvcounts,1);

    int *displs=nullptr;
    if(p->mpirank==0)
    {
        int sum=recvcounts[0]-1;
        displs = (int *)malloc(p->mpi_size*sizeof(int));
        recvcounts[0]--;
        displs[0]=1;
        for(int n=1; n<p->mpi_size; ++n)
        {
            displs[n]=sum+1;
            sum+=recvcounts[n]-1;
            recvcounts[n]--;
        }
        offsets = (int *)malloc(sum*sizeof(int));
        for(int n=0; n<54; ++n)
            offsets[n]=-1;
    }
        
    pgc->gatherv_int(offset+1,n-1,offsets,recvcounts,displs);
    if(p->mpirank==0)
    {
        offsets[0]=0;
        for(int n=1;n<p->mpi_size;++n)
        {
            for(int m=0;m<recvcounts[n];++m)
                offsets[displs[n]+m]+=offsets[displs[n]-1];
        }
    }
}

void printer_CFD::print3DMPI(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    if(MPIinitialPrint)
        prepMPIprint(p,a,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed);
    ...
    writeHeader(p,a,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed,file,piextent);

    writeData(p,a,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed,file);

    pgc->File_close(&file);

    if(MPIinitialPrint)
        MPIinitialPrint=false;
}

void printer_CFD::writeHeader(lexer *p, fdm *a, ghostcell *pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed, MPI_File &file, int *piextent)
{
    if(MPIinitialPrint)
        offsetMPI = new MPI_Offset[p->mpi_size];
    if (p->mpirank == 0)
    {
        std::ofstream headerFile(name);
        n=0;
        headerFile << "<VTKFile type=\"RectilinearGrid\" version=\"1.0\" byte_order=\"LittleEndian\">\n";
        headerFile << "  <RectilinearGrid WholeExtent=\"0 "<<p->gknox<<" 0 "<<p->gknoy<<" 0 "<<p->gknoz<<"\">\n";
        for (int i = 0; i < p->mpi_size; ++i)
        {
            if(MPIinitialPrint)
                offsetMPI[i] = offsets[n];
            headerFile << "    <Piece Extent=\"" << piextent[0+6*i] << " " << piextent[1+6*i] << " " << piextent[2+6*i] << " " << piextent[3+6*i] << " " << piextent[4+6*i] << " " << piextent[5+6*i] << "\">\n";
            pointData(p,a,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed,headerFile,offsets);
            headerFile << "      <Coordinates>\n";
            headerFile << "        <DataArray Name=\"x_coords\" type=\"Float32\" format=\"appended\" offset=\"" << offsets[n] << "\"/>\n"; n++;
            headerFile << "        <DataArray Name=\"y_coords\" type=\"Float32\" format=\"appended\" offset=\"" << offsets[n] << "\"/>\n"; n++;
            headerFile << "        <DataArray Name=\"z_coords\" type=\"Float32\" format=\"appended\" offset=\"" << offsets[n] << "\"/>\n"; n++;
            headerFile << "      </Coordinates>\n";
            headerFile << "    </Piece>\n";
        }
        headerFile << "  </RectilinearGrid>\n";
        headerFile << "  <AppendedData encoding=\"raw\">\n";
        headerFile << "_";
        std::streampos headerSize=headerFile.tellp();
        headerFile.seekp(headerSize+static_cast<std::streampos>(offsets[n]));
        headerFile<<endl<<"  </AppendedData>\n";
        headerFile<<"</VTKFile>";
        headerFile.close();
        if(MPIinitialPrint)
            for (int i = 0; i < p->mpi_size; ++i)
                offsetMPI[i] += headerSize;
        }
    if(MPIinitialPrint)
        pgc->Bcast(offsetMPI, p->mpi_size, MPI_OFFSET);
}

void printer_CFD::writeData(lexer *p, fdm *a, ghostcell *pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed, MPI_File &file)
{
    ...
    pgc->File_write_at_all_float(file, offsetMPI[p->mpirank], data.data(), data.size());
}

void printer_CFD::offsetsComp(lexer *p, fdm* a, ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    std::ofstream result;
    n=0;
...
void printer_CFD::pointData(lexer *p, fdm* a, ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed, std::ofstream &result, int *offset)
{
    result<<"      <PointData>"<<endl;
    result<<"        <DataArray type=\"Float32\" Name=\"velocity\" NumberOfComponents=\"3\" format=\"appended\" offset=\""<<offset[n]<<"\" />"<<endl;
    ++n;
        ...
        // ---------------------------------------------------------
        {
            ...
            for(int n=0;n<p->mpi_size;++n)
            {
                ...
                for(int k=0;k<(gbeginEndPoint[5+6*n]-gbeginEndPoint[4+6*n]);k++)
                {
                    ...
                    {
                        offsetCMPI.push_back(*(offsetCMPI.end()-1)+3*sizeof(float)*(p->gknox+1));
                        if(n==0&&k==0&&j==0)
            ...
            //  Velocities
            {
                ...
                for(int k=kbeginPoint;k<kendPoint;++k)
                    for(int j=jbeginPoint;j<jendPoint;++j)
                    {
                        ...
                        // pgc->File_write_at_all_char(file, offsetCMPI[offsetCMPIitr[m*p->mpi_size+p->mpirank]+n], result.str().c_str(), result.str().size());
