...
void nhflow_print_wsfline::start(lexer *p, fdm_nhf *d, ghostcell *pgc, ioflow *pflow, slice &f)
{
    ...
    {
        // open file
        sprintf(name,"./REEF3D_NHFLOW_WSFLINE/REEF3D-NHFLOW-wsfline-%0*i.dat",p->padding_time,num);
...
void nhflow_print_wsfline::sort(double *a, double *b, int *c, int left, int right)
{
...
void nhflow_print_wsfline::remove_multientry(lexer *p, double* b, double* c, int *d, int& num)
{
    int oldnum=num;
    double xval=-1.12e23;
...
int nhflow_print_wsfline::conv(double a)
{
