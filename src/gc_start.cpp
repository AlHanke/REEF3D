...
void ghostcell::start1(lexer *p, field& f, int gcv)
{
    ...
    starttime=timer();
    QQGC1LOOP
    if((p->gcb1[p->level][qq].cs!=Y_POS && p->gcb1[p->level][qq].cs!=Y_NEG) || p->j_dir==1)
                           ...
                           std::initializer_list<std::pair<field_amrex*, int>> fields_and_gcvs)
{
    ...
    starttime=timer();
    field_amrex::FillDomainBoundaryBatch(p, shared_mf, scomp, fields_and_gcvs, m_d_bcrec_batch, !p->j_dir);
