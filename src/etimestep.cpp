...
etimestep::etimestep(lexer*) : epsi(1.0e-19)
{
}
...
void etimestep::start(fdm *a, lexer *p, ghostcell *pgc, turbulence *pturb)
{
    ...
    if(p->N50==1)
    {
        LOOP
        {
            double dx = std::min({p->DXP[IP],p->DYN[JP],p->DZN[KP]});

            double visc = 0.5*(a->eddyv(i,j,k) + a->eddyv(i+1,j,k)) + 0.5*(a->visc(i,j,k) + a->visc(i+1,j,k));

            cu = std::min(cu, 2.0/((sqrt(p->umax*p->umax + p->vmax*p->vmax + p->wmax*p->wmax)/dx +  visc*(6.0/pow(dx,2.0)))
                              + sqrt(pow(sqrt(p->umax*p->umax + p->vmax*p->vmax + p->wmax*p->wmax)/dx+visc,2.0)
                              + (4.0*fabs(std::max({a->maxF,a->maxG,a->maxH})))/dx)));
        }
    }
    else if(p->N50==2)
    {
        LOOP
        {
            ...
            cu = std::min(cu, 2.0/((sqrt(p->umax*p->umax)/p->DXP[IP] + visc*(6.0/pow(p->DXP[IP],2.0)))
                              + sqrt(pow(sqrt(p->umax*p->umax)/p->DXP[IP]+visc,2.0)
                              + (4.0*fabs(a->maxF))/p->DXN[IP])));


            cv = std::min(cv, 2.0/((sqrt(p->vmax*p->vmax)/p->DYN[JP] + visc*(6.0/pow(p->DYN[JP],2.0)))
                              + sqrt(pow(sqrt(p->vmax*p->vmax)/p->DYN[JP]+visc,2.0)
                              + (4.0*fabs(a->maxG))/p->DYN[JP])));


            cw = std::min(cw, 2.0/((sqrt(p->wmax*p->wmax)/p->DZN[KP] + visc*(6.0/pow(p->DZN[KP],2.0)))
