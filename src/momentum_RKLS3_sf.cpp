...
void momentum_RKLS3_sf::starti(lexer* p, fdm* a, ghostcell* pgc, sixdof* p6dof, vrans* pvrans, fsi* pfsi)
{
    ...
    // Forcing
        ...
        ULOOP
        {
            ...
            p->sfmax = std::max(fabs(2.0*alpha(loop)*CPOR1*fx(i,j,k)), p->sfmax);
        }
        ...
        VLOOP
        {
            ...
            p->sfmax = std::max(fabs(2.0*alpha(loop)*CPOR2*fy(i,j,k)), p->sfmax);
        }
        ...
        WLOOP
        {
            ...
            p->sfmax = std::max(fabs(2.0*alpha(loop)*CPOR3*fz(i,j,k)), p->sfmax);
        }
...
void momentum_RKLS3_sf::irhs(lexer *p, fdm *a)
{
    n=0;
...
void momentum_RKLS3_sf::jrhs(lexer *p, fdm *a)
{
    n=0;
...
void momentum_RKLS3_sf::krhs(lexer *p, fdm *a)
{
    n=0;
