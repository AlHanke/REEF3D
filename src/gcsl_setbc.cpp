...
void ghostcell::gcsl_setbc1(lexer *p)
{
    ...
    GCSL1LOOP
    {
        ...
        if(cs==X_NEG && bc==WALL && i+p->origin_i==0)
        p->gcbsl1[n][4]=p->bcside1;

        if(cs==X_POS && bc==WALL && i+p->origin_i==p->gknox-2)
        p->gcbsl1[n][4]=p->bcside4;

        if(cs==Y_NEG && bc==WALL && j+p->origin_j==0)
        p->gcbsl1[n][4]=p->bcside3;

        if(cs==Y_POS && bc==WALL && j+p->origin_j==p->gknoy-1)
...
void ghostcell::gcsl_setbc2(lexer *p)
{
    ...
    GCSL2LOOP
    {
        ...
        if(cs==X_POS && bc==WALL && i+p->origin_i==p->gknox-1)
        p->gcbsl2[n][4]=p->bcside4;

        if(cs==Y_NEG && bc==WALL && j+p->origin_j==0)
        p->gcbsl2[n][4]=p->bcside3;

        if(cs==Y_POS && bc==WALL && j+p->origin_j==p->gknoy-2)
