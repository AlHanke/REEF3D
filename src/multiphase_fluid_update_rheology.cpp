...
multiphase_fluid_update_rheology::multiphase_fluid_update_rheology(lexer *p) : gcval_ro(1),gcval_visc(1),ro1(p->W1),visc1(p->W2),ro2(p->W3),visc2(p->W4),ro3(p->W6),visc3(p->W7)
{
    if(p->W90>0)
...
void multiphase_fluid_update_rheology::start(lexer *p, fdm* a, ghostcell* pgc, field &ls1, field &ls2)
{
    ...
    double epsi = p->F45*(1.0/(2.0+p->j_dir))*(p->DRM+(p->j_dir==1?p->DSM:0)+p->DTM);
