...
hires::hires (lexer *p, int limiter) : plim(std::in_place_type<minmod>, p)
{
    if(p->j_dir==0)
    {
        if(p->B200>=1 || p->S10==2)
    ...
    if(limiter==10)
    plim.emplace<minmod>(p);

    else if(limiter==11)
    plim.emplace<vanleer>(p);

    else if(limiter==12)
    plim.emplace<umist>(p);

    else if(limiter==13)
    plim.emplace<vanalbada>(p);

    else if(limiter==14)
    plim.emplace<superbee>(p);

    else if(limiter==15)
    plim.emplace<smart>(p);

    else if(limiter==16)
    plim.emplace<limo3>(p);

    else if(limiter==42)
    plim.emplace<tvdvof>(p);
}

void hires::start(lexer *p, fdm *a, field &b, int ipol, field &uvel, field &vvel, field &wvel)
{
    std::visit([&](auto& flux, auto& lim)
    {
        if(ipol==1)
        {
            FIELDLOOP_INC_MEMBER(a,F,
                FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
                F(i,j,k)+=aij(flux,lim,p,a,b,1,uvel,vvel,wvel,p->DXP.data(),p->DYN.data(),p->DZN.data());
            )
        }
        else if(ipol==2 && p->j_dir==1)
        {
            FIELDLOOP_INC_MEMBER(a,G,
                FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
                G(i,j,k)+=aij(flux,lim,p,a,b,2,uvel,vvel,wvel,p->DXN.data(),p->DYP.data(),p->DZN.data());
            )
        }
        else if(ipol==3)
        {
            FIELDLOOP_INC_MEMBER(a,H,
                FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
                H(i,j,k)+=aij(flux,lim,p,a,b,3,uvel,vvel,wvel,p->DXN.data(),p->DYN.data(),p->DZP.data());
            )
        }
        else if(ipol==4)
        {
            FIELDLOOP_INC_MEMBER(a,L,
                FIELD_CONST_INC(b); FIELD_CONST_INC(uvel); FIELD_CONST_INC(vvel); FIELD_CONST_INC(wvel),
                L(i,j,k)+=aij(flux,lim,p,a,b,4,uvel,vvel,wvel,p->DXN.data(),p->DYN.data(),p->DZN.data());
            )
        }
    }, pflux, plim);
}
...
double hires::aij(FluxT &pflux, LimT &plim, lexer *p, fdm *a, const GenericField &b, int ipol, const GenericField &uvel, const GenericField &vvel, const GenericField &wvel, double *DX, double *DY, double *DZ)
{
