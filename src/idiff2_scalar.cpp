...
void idiff2::idiff_scalar(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &b, field &eddyv, double sig, double alpha)
{
    ...
    LOOP
    {
        ...
        a->M.p[count] +=  0.5*(a->visc(i+1,j,k)+a->eddyv(i+1,j,k)/sig + visc_ijk+ev_ijk/sig)/(p->DXN[IP]*p->DXP[IM1])
                      ...
                      +   (p->j_dir ? 0.5*(a->visc(i,j+1,k)+a->eddyv(i,j+1,k)/sig + visc_ijk+ev_ijk/sig)/(p->DYN[JP]*p->DYP[JM1])
                      +   0.5*(visc_ijk+ev_ijk/sig + a->visc(i,j-1,k)+a->eddyv(i,j-1,k)/sig)/(p->DYN[JP]*p->DYP[JP]) : 0.0)
        ...
        if(p->j_dir)
        {
            a->M.e[count] -= 0.5*(visc_ijk+ev_ijk/sig + a->visc(i,j-1,k)+a->eddyv(i,j-1,k)/sig)/(p->DYN[JP]*p->DYP[JM1]);
            a->M.w[count] -= 0.5*(a->visc(i,j+1,k)+a->eddyv(i,j+1,k)/sig + visc_ijk+ev_ijk/sig)/(p->DYN[JP]*p->DYP[JP]);
        }
