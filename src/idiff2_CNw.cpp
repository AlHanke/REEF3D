...
void idiff2_CN::diff_w(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &diff, field &w_in, field &u, field &v, field &w, double alpha)
{
	...
	visc_ddy_m = 0.25*(visc_i_jm_k+ev_i_jm_k + a->visc(i,j-1,k+1)+a->eddyv(i,j-1,k+1) + visc_ijk+ev_ijk + visc_i_j_kp+ev_i_j_kp);
        ...
        a->M.p[count] = alphaCN*visctermp + CPOR3/(alpha*(p->dt));

        a->rhsvec.V[count] -= (1.0-alphaCN)*visctermp*w_in(i,j,k);
        ...
        a->rhsvec.V[count] +=  (((a->u(i,j,k+1)-u(i,j,k))*visc_ddx_p) - ((u(i-1,j,k+1)-u(i-1,j,k))*visc_ddx_m))/(p->DZP[KP]*p->DXN[IP])
         ...
         visctermb = -2.0*(visc_ijk+ev_ijk)/(p->DZN[KP]*p->DZP[KP]);
         visctermt = -2.0*(visc_i_j_kp+ev_i_j_kp)/(p->DZN[KP1]*p->DZP[KP]);
