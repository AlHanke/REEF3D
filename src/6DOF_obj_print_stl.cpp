...
void sixdof_obj::print_stl(lexer *p, ghostcell *pgc)
{
    ...
    if(p->mpirank==0 && printflag==1)
    {
        ...
        if(p->A10==2)
        sprintf(path,"./REEF3D_SFLOW_6DOF_STL/REEF3D-6DOF-%i-%0*i.stl",n6DOF,p->padding_mpi,num);
        
        if(p->A10==5)
        sprintf(path,"./REEF3D_NHFLOW_6DOF_STL/REEF3D-6DOF-%i-%0*i.stl",n6DOF,p->padding_mpi,num);
        
        if(p->A10==6)
        sprintf(path,"./REEF3D_CFD_6DOF_STL/REEF3D-6DOF-%i-%0*i.stl",n6DOF,p->padding_mpi,num);
