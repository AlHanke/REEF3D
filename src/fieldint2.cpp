...
void fieldint2::fieldalloc(lexer* p)
{
	int gridsize = p->imax*p->jmax*p->kmax;
	p->Iarray(V,gridsize);
}

void fieldint2::fieldgcalloc(lexer* p)
{
...
void fieldint2::resize(lexer* p)
{
    int factor=3;
    
	p->Iresize(gcfeld,gcfeldsize, p->gcextra2*factor, 6, 6, 4, 4); 
	gcfeldsize=p->gcextra2*factor;
}

//------------------------------------------------------------------------------

int & fieldint2::operator()(int ii, int jj, int kk)
{	
