...
void ghostcell::start20V(lexer *p, double *f, int gcv) //KIN
{
    ...
    if(p->DF(i,j,k)>0)
    {
        ...
        // xxxxxxx
        // s
        if((p->flag4[Im1JK]<0 && inflow==0) || (p->DF(i-1,j,k)<0))
        {
            if(p->B11==1)
            {
                f[Im1JK] = f[IJK];
        ...
        // n
        if((p->flag4[Ip1JK]<0 && outflow==0) || (p->DF(i+1,j,k)<0))
        {
            if(p->B11==1)
            {
                f[Ip1JK] = f[IJK];
        ...
        // yyyyy
        if((p->flag4[IJm1K]<0  || (p->DF(i,j-1,k)<0)) && p->j_dir==1)
        {
            if(p->B11==1)
            {
                f[IJm1K] = f[IJK];
        ...
        if((p->flag4[IJp1K]<0  || (p->DF(i,j+1,k)<0)) && p->j_dir==1)
        {
            if(p->B11==1)
            {
                f[IJp1K] = f[IJK];
        ...
        // zzzzz
        if(p->flag4[IJKp1]<0  || (p->DF(i,j,k+1)<0) || k==p->knoz-1)
        {
        ...
        // bed
        if(p->flag4[IJKm1]<0  || (p->DF(i,j,k-1)<0 && p->B11==1)   || (k==0 && p->B11==1))
        {
            if(p->B11==1)
            {
                f[IJKm1] = f[IJK];
...
void ghostcell::start30V(lexer *p, double *f, int gcv) // EPS
{
    ...
    if(p->DF(i,j,k)>0)
    {
        ...
        // bed
        if(p->flag4[IJKm1]<0  || (p->DF(i,j,k-1)<0) || k==0)
        {
            f[IJKm1] = f[IJK];
            f[IJKm2] = f[IJK];
            f[IJKm3] = f[IJK];
