...
v    id sflow_momentum_RK2::start(lexer *p, fdm2D* b, ghostcell* pgc)
{            
...
//--------------------------------------------------------
    ...
    SLICELOOP1
                P(i,j) = 0.5*b->P(i,j) + 0.5*Prk1(i,j)
                + 0.5*p->dt*b->F(i,j);
    ...
    SLICELOOP2
            >Q(i,j) = 0.5*b->Q(i,j) + 0.5*Qrk1(i,j)
              + 0.5*p->dt*b->G(i,j);
    ...
    pdiff->diff_w(p,b,pgc,psolv,Prk1,Qrk1,wrk1,0.5);
       ...
       SLICELOOP4
            >ws(i,j) = 0.5*b->ws(i,j) + 0.5*wrk1(i,j)
              + 0.5*p->dt*b->L(i,j);
