...
double interpolation::nhf_ipol4eta(int *wet,slice &f, slice &bed)
{
    double bedvalue;
    double wd_criterion=0.00005;
    int jj;
    ...
    if(wet[IJ]==0)
    v1=MIN(f(i,j),bed(i,j)-p->wd-fac*p->DXM);

    if(wet[Ip1J]==1)
    v2=f(i+1,j);
    
    if(wet[Ip1J]==0)
    v2=MIN(f(i+1,j),bed(i+1,j)-p->wd-fac*p->DXM);
    ...
    if(wet[IJp1]==0)
    v3=MIN(f(i,j+1),bed(i,j+1)-p->wd-fac*p->DXM);

    if(wet[Ip1Jp1]==1)
    v4=f(i+1,j+1);
    
    if(wet[Ip1Jp1]==0)
    v4=MIN(f(i+1,j+1),bed(i+1,j+1)-p->wd-fac*p->DXM);
