...
public:
    ...
    void u_flux(fdm*, int ipol, const field& uvel, double &uflux1, double&)
    { u_flux_impl(ipol, uvel, uflux1); }

    void v_flux(fdm*, int ipol, const field& vvel, double &vflux1, double&)
    { vflux1 = 0.0; }

    void w_flux(fdm*, int ipol, const field& wvel, double &wflux1, double&)
    { w_flux_impl(ipol, wvel, wflux1); }

    #if USE_AMREX
    void u_flux(fdm*, int ipol, const LocalArr4Const& uvel, double &uflux1, double&)
    { u_flux_impl(ipol, uvel, uflux1); }

    void v_flux(fdm*, int ipol, const LocalArr4Const& vvel, double &vflux1, double&)
    { vflux1 = 0.0; }

    void w_flux(fdm*, int ipol, const LocalArr4Const& wvel, double &wflux1, double&)
    { w_flux_impl(ipol, wvel, wflux1); }
    #endif
