...
void net_barDyn::updateField_nhflow(lexer *p, fdm_nhf *d, ghostcell *pgc, int cmp)
{
    int *recField, *count;
    ...
    for (int i = 0; i < nK; i++)
    {
        ...
        )
        {
            if (cmp==0)
            {
                coupledField[i][cmp] = p->ccipol4V(d->U,d->WL,d->bed,x_(i,0),x_(i,1),x_(i,2));
            }
            else if (cmp==1)
            {
                coupledField[i][cmp] = p->ccipol4V(d->V,d->WL,d->bed,x_(i,0),x_(i,1),x_(i,2));
            }
            else if (cmp==2)
            {
                coupledField[i][cmp] =  p->ccipol4V(d->W,d->WL,d->bed,x_(i,0),x_(i,1),x_(i,2));
            }
            else if (cmp==3)
            {
