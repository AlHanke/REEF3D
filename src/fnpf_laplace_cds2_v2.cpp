...
void fnpf_laplace_cds2_v2::start(lexer* p, fdm_fnpf *c, ghostcell *pgc, solver *psolv, fnpf_fsf *pf, double *f, slice &Fifsf)
{
    p->poissoniter=0;
    p->poissontime=0.0;
    ...
    FLOOP
    {
        if(p->wet[IJ]==1 && p->flag7[FIJK]>0 && k<p->knoz)
        {
            ...
            c->M.w[n] = p->j_dir ? -1.0/(p->DYP[JP]*p->DYN[JP]) : 0.0;
            c->M.e[n] = p->j_dir ? -1.0/(p->DYP[JM1]*p->DYN[JP]) : 0.0;
            ...
            c->rhsvec.V[n] =  2.0*p->sigx[FIJK]*(f[FIp1JKp1] - f[FIm1JKp1] - f[FIp1JKm1] + f[FIm1JKm1])
                            ...
                            + (p->j_dir ? 2.0*p->sigy[FIJK]*(f[FIJp1Kp1] - f[FIJm1Kp1] - f[FIJp1Km1] + f[FIJm1Km1])
                            /((p->DYP[JP]+p->DYP[JM1])*(p->DZN[KP]+p->DZN[KM1])) : 0.0);
        }

        if(p->wet[IJ]==0 || p->flag7[FIJK]<0 || k==p->knoz)
        {
            c->M.p[n]  =  1.0;
    ...
    FLOOP
    {
        if(p->wet[IJ]==1 && p->flag7[FIJK]>0 && k<p->knoz)
        {
            ...
            if(p->B99>2)
            {
                ...
                {
                    c->rhsvec.V[n] -= (p->i_dir ? 2.0*p->sigx[FIJK]*(f[FIp1JKp1] - f[FIm1JKp1] - f[FIp1JKm1] + f[FIm1JKm1])
                                /((p->DXP[IP]+p->DXP[IM1])*(p->DZN[KP]+p->DZN[KM1])) : 0.0);

                    c->rhsvec.V[n] += (p->i_dir ? 2.0*p->sigx[FIJK]*(c->Uin[FIp1JKp1] - c->Uin[FIp1JKm1])
                                /((p->DZN[KP]+p->DZN[KM1])) : 0.0);
    ...
    starttime=pgc->timer();
    psolv->startF(p,pgc,f,c->rhsvec,c->M,7);
    endtime=pgc->timer();
