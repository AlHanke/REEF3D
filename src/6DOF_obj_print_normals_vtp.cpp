...
void sixdof_obj::print_normals_vtp(lexer *p, ghostcell *pgc)
{
    ...
    if(p->mpirank==0 && (((p->count%p->P20==0) && p->P30<0.0)  || (p->simtime>printtime && p->P30>0.0)   || p->count==0))
    {
        ...
        if(p->A10==5)
        sprintf(path,"./REEF3D_NHFLOW_6DOF_Normals_VTP/REEF3D-6DOF-Normals-%i-%0*i.vtp",n6DOF,p->padding_mpi,num);

        if(p->A10==6)
        sprintf(path,"./REEF3D_CFD_6DOF_Normals_VTP/REEF3D-6DOF-Normals-%i-%0*i.vtp",n6DOF,p->padding_mpi,num);
