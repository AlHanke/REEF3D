...
ghostcell::bc_labels ghostcell::gceval3(lexer *p, int gcv, int bc, int cs)
{
    ...
    // Othogonal
    else if(((bc==7 && !awa_label) || bc==5 || bc==21 || bc==22) && (cs==dir_labels::Z_POS || (cs==dir_labels::Z_NEG && (p->A10==5 || p->A10==6))) && gcv==12)
    ...
    //Free Surface
    else if(bc==3 && (cs==dir_labels::X_NEG || cs==dir_labels::X_POS || cs==dir_labels::Y_NEG || cs==dir_labels::Y_POS) && gcv==12)
        return bc_labels::NEUMANN;

    else if(bc==3 && (cs==dir_labels::Z_NEG || cs==dir_labels::Z_POS) && gcv==12 && p->A10!=3 && p->A10!=5)
        return bc_labels::NOSLIP;

    else if(bc==3 && (cs==dir_labels::Z_NEG || cs==dir_labels::Z_POS) && gcv==12 && p->A10==3)
        return bc_labels::NEUMANN;

    else if(bc==3 && (cs==dir_labels::Z_NEG || cs==dir_labels::Z_POS) && (gcv==12 || gcv==112) && p->A10==5)
        return bc_labels::NHPRESS;

    else if(bc==9 && cs==dir_labels::Z_POS && gcv==12)
        return bc_labels::NEUMANN;
