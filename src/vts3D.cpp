...
vts3D::vts3D(lexer* p, fdm *a, ghostcell *pgc) : nodefill(p), eta(p)
{
    if(p->F50==1)
	gcval_phi=51;
...
void vts3D::ini(lexer* p, fdm* a, ghostcell* pgc)
{
}

void vts3D::start(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, ioflow *pflow, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
...
void vts3D::print_stop(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, ioflow *pflow, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    print_vtu(a,p,pgc,pturb,pheat,pflow,psolv,pdata,pconc,pmp,psed);
    
}

void vts3D::print_vtu(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, ioflow *pflow, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    if(p->P180==1)
	pfsf->start(p,a,pgc);
...
void vts3D::print3D(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
	...
	//points
    offset[n]=offset[n-1]+4+3*4*(p->pointnum);//4*(p->knox+1)*(p->knoy+1)*(p->knoz+1); 
    ++n;
	//---------------------------------------------
...
//  walldist
	...
	// Coordinates
	// points
	iin=3*4*(p->knox+1)*(p->knoy+1)*(p->knoz+1);
	iin=4*(p->pointnum);
	result.write((char*)&iin, sizeof (int));
	int count=0;
    KTLOOP
	...
	cout<<p->mpirank<<"|"<<count<<"|"<<(p->knox+1)*(p->knoy+1)*(p->knoz+1)<<"|"<<iin/4.0<<"|"<<p->pointnum<<endl;
