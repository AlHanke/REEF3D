...
void idiff2_CN::diff_u(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &u, field &v, field &w, double alpha)
{
	...
	visctermp =  2.0*(visc_ip_j_k+ev_ip_j_k)/(p->DXN[IP1]*p->DXP[IP])
        ...
        a->M.p[count] = alphaCN*visctermp + CPOR1/(alpha*(p->dt));

        a->rhsvec.V[count] -= (1.0-alphaCN)*visctermp*u(i,j,k);
	...
	viscterms = -2.0*(visc_ijk+ev_ijk)/(p->DXN[IP]*p->DXP[IP]);
        visctermn = -2.0*(visc_ip_j_k+ev_ip_j_k)/(p->DXN[IP1]*p->DXP[IP]);
        viscterme = -visc_ddy_m/(p->DYP[JM1]*p->DYN[JP]);
        visctermw = -visc_ddy_p/(p->DYP[JP]*p->DYN[JP]);
	...
	a->M.t[count] = alphaCN*visctermt;
        ...
        a->rhsvec.V[count] -= (1.0-alphaCN)*(viscterms*u(i-1,j,k) + visctermn*u(i+1,j,k) + viscterme*u(i,j-1,k) + visctermw*u(i,j+1,k) + visctermb*u(i,j,k-1) + visctermt*u(i,j,k+1)); 
...
void idiff2_CN::diff_u(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &diff, field &u_in, field &u, field &v, field &w, double alpha)
{
	starttime=pgc->timer();
	double visc_ddy_p,visc_ddy_m,visc_ddz_p,visc_ddz_m;
        double visctermp, viscterms, visctermn, viscterme;
	...
	visc_ddz_m = 0.25*(visc_i_j_km+ev_i_j_km + a->visc(i+1,j,k-1)+a->eddyv(i+1,j,k-1) + visc_ijk+ev_ijk + visc_ip_j_k+ev_ip_j_k);
        ...
        a->rhsvec.V[count] -= (1.0-alphaCN)*visctermp*u_in(i,j,k);
    ...
    p->uiter=p->solveriter;
    if(p->mpirank==0 && p->D21==1 && (p->count%p->P12==0))
    cout<<"udiffiter (CN): "<<p->uiter<<"  udifftime (CN): "<<setprecision(3)<<time<<endl;
