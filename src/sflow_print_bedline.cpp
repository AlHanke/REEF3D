...
void sflow_print_bedline::start(lexer *p, fdm2D *b, ghostcell *pgc, ioflow *pflow, slice &f)
{
    ...
    if(p->mpirank==0)
    {
        ...
        {
        if(num<10)
        sprintf(name,"REEF3D-SFLOW-bedline-00000%i.dat",num);

        if(num<100&&num>9)
        sprintf(name,"REEF3D-SFLOW-bedline-0000%i.dat",num);

        if(num<1000&&num>99)
        sprintf(name,"REEF3D-SFLOW-bedline-000%i.dat",num);

        if(num<10000&&num>999)
        sprintf(name,"REEF3D-SFLOW-bedline-00%i.dat",num);

        if(num<100000&&num>9999)
        sprintf(name,"REEF3D-SFLOW-bedline-0%i.dat",num);

        if(num>99999)
        sprintf(name,"REEF3D-SFLOW-bedline-%i.dat",num);
        }
...
void sflow_print_bedline::sort(double *a, double *b, int *c, int left, int right)
{
...
void sflow_print_bedline::remove_multientry(lexer *p, double* b, double* c, int *d, int& num)
{
    int oldnum=num;
    double xval=-1.12e23;
...
int sflow_print_bedline::conv(double a)
{
