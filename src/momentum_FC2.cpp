...
momentum_FC2::momentum_FC2(lexer *p, fdm *a, ghostcell *pgc, convection *pconvection, convection *ppfsfdisc, diffusion *pdiffusion, pressure* ppressure, poisson* ppoisson,
                                                    ...
                                                    fsi *ppfsi)
                                                    :momentum_forcing(p),bcmom(p),udiff(p),urk1(p),fx(p),vdiff(p),vrk1(p),fy(p),wdiff(p),wrk1(p),fz(p),
                                                    ls(p),frk1(p)
{
	gcval_u=10;
	gcval_v=11;
...
//--------------------------------------------------------
	...
	pflow->pressure_io(p,a,pgc);
	ppress->start(p,a,pgc,ppois,ppoissonsolv,pflow, a->u, a->v,a->w,0.5);
...
void momentum_FC2::utimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_FC2::vtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_FC2::wtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}
