...
void sflow_pjm_lin::poisson(lexer*p, fdm2D* b, double alpha)
{
    ...
    SLICELOOP4
    {
        b->M.p[n] = (p->i_dir ? (b->hp(i,j)*sqd + b->hp(i,j)*sqd) : 0.0)
                  + (p->j_dir ? (b->hp(i,j)*sqd + b->hp(i,j)*sqd) : 0.0)
                  + 2.0/(HP*p->W1);

        if(p->i_dir)
        {
            b->M.n[n] = -b->hp(i,j)*sqd;
            b->M.s[n] = -b->hp(i,j)*sqd;
        }
        else
        {
            b->M.n[n] = 0.0;
        ...
        if(p->j_dir)
        {
            b->M.w[n] = -b->hp(i,j)*sqd;
            b->M.e[n] = -b->hp(i,j)*sqd;
        }
        else
        {
            b->M.w[n] = 0.0;
