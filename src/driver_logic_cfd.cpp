...
// Free Surface
    ...
    if(p->F30==2  && p->N40!=22 && p->N40!=23 && p->N40!=33)
    pfsf = new levelset_RK2(p,a,pgc,pheat,pconc);

    if(p->F30==3  && p->N40!=22 && p->N40!=23 && p->N40!=33)
    ...
    //pressure scheme + poisson scheme for pressure
    if(p->D30==0)
        ppress = new pressure_void();
    else if(p->D30==1 || p->D30==2 || p->D30==3)
    {
        if(p->F10==1)
            ppress = new pjm_nse(pd);
        else if(p->F10==2)
...
// Velocities
    ...
    if(p->N40==1)
    pmom = new momentum_AB2(p,a,pconvec,pdiff,ppress,pturb,psolv,ppoissonsolv,pflow);
