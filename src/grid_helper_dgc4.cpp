...
void grid_helper::fill_dgc4(lexer* p)
{
    ...
    QGC4LOOP
    {
        ...
        if(p->gcb4[p->level][q].cs==X_NEG)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin-n-1)*jmax*kmax + (j-jmin)*kmax + k-kmin]+=1;

        else if(p->gcb4[p->level][q].cs==X_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin+n+1)*jmax*kmax + (j-jmin)*kmax + k-kmin]+=1;

        else if(p->gcb4[p->level][q].cs==Y_NEG)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin-n-1)*kmax + k-kmin]+=1;

        else if(p->gcb4[p->level][q].cs==Y_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin+n+1)*kmax + k-kmin]+=1;

        else if(p->gcb4[p->level][q].cs==Z_NEG)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin)*kmax + k-kmin-n-1]+=1;

        else if(p->gcb4[p->level][q].cs==Z_POS)
            for(n=0;n<p->margin;++n)
                hgc[(i-imin)*jmax*kmax + (j-jmin)*kmax + k-kmin+n+1]+=1;
    }
    GC_TILE_RESET;
    ...
    p->Iresize(p->dgc4,p->dgc4_count,count,6,6);
