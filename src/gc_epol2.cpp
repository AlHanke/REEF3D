...
ghostcell::bc_labels ghostcell::gceval2(lexer *p, int gcv, int bc, int cs)
{
    ...
    // Inflow
    else if(bc==gbc_labels::WAVEGEN && (gcv==8 || gcv==11))
        return gclabel_v_in;
...
void ghostcell::gcdistro2(field &f, int ii, int jj, int kk, int cs, int bc, int gcv)
{
    gcdistro(f,ii,jj,kk,gceval2(p,gcv,bc,cs),cs);
}
