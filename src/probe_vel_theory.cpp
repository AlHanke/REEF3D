...
probe_vel_theory::probe_vel_theory(lexer *p, fdm *a) : probenum(p->P66)
{
    ...
    if(p->mpirank==0 && probenum>0)
    {
		...
		{
		snprintf(name,sizeof(name),"./REEF3D_CFD_ProbePoint/REEF3D-CFD-Vel-Probe-Theory-%i.dat",n+1);
...
probe_vel_theory::~probe_vel_theory()
{
    for(n=0;n<probenum;++n)
    pout[n].close();
...
void probe_vel_theory::start(lexer *p, fdm *a, ghostcell *pgc, ioflow *pflow)
{
    double xp,yp,zp;
